...
Database.prototype.put = function (id, doc, callback) {
  ...
  this.query({
    ...
    if (e) {
      ...
      logger.error("ERROR IN PUT for doc id " + id)
      callback(e)
    } else {
  ...
  if (Array.isArray(doc)) {
    ...
    if (id) {
      ...
      if (document._rev) {
        ...
        var self = this;
        this.cache.get(id, function (err, doc) {
          if (err) {
            logger.error(util.inspect(err))
            // Handle error here. So not in cache, query the db instead.
