...
Database.prototype.get = function (id, rev) {
    ...
    if (Array.isArray(id)) { // Bulk GET
        ...
        if (rev && args.length === 2) {
            ...
            var self = this;
            this.cache.has(id, function (err, result) {
                if (err) {
                    ...
                    self.query({
                        ...
                        args.callback(err, res.json);
                    });
                } else {
                    if (result === true) {
                        self.cache.get(id, function (err, doc) {
                            if (err) {
                                logger.error(util.inspect(err))
                                self.query({
                                    path: cradle.escape(id),
    ...
    if (Array.isArray(doc)) {
        ...
        if (id) {
            ...
            if (document._rev) {
                ...
                this.cache.has(id, function (err, result) {
                    if (err) {
                        logger.error("IN _SAVE FUNCTION 1")
                        logger.error(util.inspect(err))
                        // self.put(id, document, callback)
                    } else {
                        if (result === true) {
                            self.cache.get(id, function (err, doc) {
                                if (err) {
                                    logger.error(util.inspect(err))
                                    logger.error("IN _SAVE FUNCTION 2")
                                    ...
                                    self.put(id, document, function (e, res) {
                                        if (e && e.headers && e.headers.status === 409) { // Conflict
                                            logger.error("IN _SAVE FUNCTION 5")
                                    ...
                                    if (typeof doc !== "undefined") {
                                        ...
                                        logger.error("IN _SAVE FUNCTION 3")
                                        // Attempt to create a new document. If it fails,
                                        // because an existing document with that _id exists (409),
                                        // perform a HEAD, to get the _rev, and try to re-save.
                                        self.put(id, document, function (e, res) {
                                            if (e && e.headers && e.headers.status === 409) { // Conflict
                                                logger.error("IN _SAVE FUNCTION 6")
                                                that.head(id, function (e, headers, res) {
                                                    if (res === 404 || !headers['etag']) {
                                                        return callback({ reason: 'not_found' });
                                                    }
...
Database.prototype.remove = function (id, rev) {
    ...
    if (typeof(rev) !== 'string') {
        ...
        this.cache.get(id, function (err, result) {
            if (err) {
                logger.error(util.inspect(err))
            } else {
                if (doc === result) {
